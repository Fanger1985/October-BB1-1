...
// Positions
float panAngle = 90;  // Adjusted center pan (now at 90°)
...
float targetPanAngle = 90;  // Updated target pan to center at 90°
...
float panSpeed = 3;   // Adjust the pan speed dynamically
float tiltSpeed = 3;  // Adjust the tilt speed dynamically
float panAcceleration = 0.4;  // Adjust pan acceleration
float tiltAcceleration = 0.4; // Adjust tilt acceleration
...
// Function to start Demo 1 sequence
void startDemo1() {
    isDemo1Running = true;
    demo1Millis = millis();
}
        ...
        switch (step) {
            ...
            case 1:
                ...
                if (currentMillis - stepStartTime >= 1000) {
                    startMovement(targetPanAngle, 70, targetRightEarAngle, targetLeftEarAngle);
                    step++;
                }
                break;
            case 2:
            ...
            case 4:
                ...
                if (currentMillis - stepStartTime >= 4000) {
                    startMovement(100, targetTiltAngle, targetRightEarAngle, targetLeftEarAngle);
                    step++;
                }
                break;
            case 5:
                if (currentMillis - stepStartTime >= 5000) {
                    startMovement(140, targetTiltAngle, targetRightEarAngle, targetLeftEarAngle);
                    step++;
                }
                break;
            case 6:
...
// Function to start Demo 2 sequence
void startDemo2() {
    isDemo2Running = true;
    demo2Millis = millis();
}
    ...
    if (currentMillis - demo2Millis >= 50) {  // Adjust timing as needed
        switch (step) {
            case 0:
                ...
                startMovement(90, 90, 90, 90);
                startCameraMovement(90, 0);
                stepStartTime = currentMillis;
                step++;
                break;
            case 1:
                // Move camera pan smoothly between 50 and 130 degrees
                if (currentMillis - stepStartTime >= 2000) {
                    startCameraMovement(50, cameraTiltAngle);
                    step++;
                }
                break;
            case 2:
                if (currentMillis - stepStartTime >= 4000) {
                    startCameraMovement(130, cameraTiltAngle);
                    step++;
                }
                break;
            case 3:
                ...
                if (currentMillis - stepStartTime >= 6000) {
                    startCameraMovement(50, 110);
                    step++;
                }
                break;
            case 4:
                if (currentMillis - stepStartTime >= 8000) {
                    startCameraMovement(130, 0);
                    step++;
                }
                break;
            case 5:
            ...
            case 7:
                ...
                break;
            case 8:
                // Pan head left to 160 degrees while moving ears
                if (currentMillis - stepStartTime >= 16000) {
                    startMovement(160, 90, 65, 45);
                    step++;
                }
                break;
            case 9:
            ...
            case 11:
                ...
                break;
            case 12:
                // Tilt head down to 85 degrees while tilting camera down and adjusting ears
                if (currentMillis - stepStartTime >= 24000) {
                    startMovement(90, 85, 145, 35);
...
void setup() {
    ...
    startMovement(90, 90, 90, 90);  // Updated initial pan to 90°
    startCameraMovement(90, 90);  // Camera pan and tilt to 90 degrees
    Serial.println("Moved to initial position on startup (Pan: 90°, Tilt: 90°, Right Ear: 90°, Left Ear: 90°, Camera Pan: 90°, Camera Tilt: 90°).");
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        html += "<div class='info'><strong>Camera Pan Angle:</strong> <span id='cameraPanAngle'>" + String(cameraPanAngle) + "</span>°</div>";
        html += "<div class='info'><strong>Camera Tilt Angle:</strong> <span id='cameraTiltAngle'>" + String(cameraTiltAngle) + "</span>°</div>";
        ...
        html += "<button class='btn' onclick=\"fetch('/start_demo1')\">Start Demo 1</button>"; // Button to start Demo 1
        html += "<button class='btn' onclick=\"fetch('/start_demo2')\">Start Demo 2</button>"; // Button to start Demo 2
        html += "</div>";
        html += "<script>";
        ...
        html += "document.getElementById('cameraPanAngle').innerText = data.cameraPan;";  // Added camera pan angle
        html += "document.getElementById('cameraTiltAngle').innerText = data.cameraTilt;"; // Added camera tilt angle
    ...
    server.on("/center", HTTP_GET, [](AsyncWebServerRequest *request){
        startMovement(90, 90, 90, 90);  // Updated center positions including ears
    ...
    server.on("/camera_pan_left", HTTP_GET, [](AsyncWebServerRequest *request){
        startCameraMovement(constrain(targetCameraPanAngle + 10, 0, 180), targetCameraTiltAngle); // Move camera pan left
        request->send(204);
    });

    server.on("/camera_pan_right", HTTP_GET, [](AsyncWebServerRequest *request){
        startCameraMovement(constrain(targetCameraPanAngle - 10, 0, 180), targetCameraTiltAngle); // Move camera pan right
        request->send(204);
    });
