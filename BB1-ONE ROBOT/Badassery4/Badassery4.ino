...
// Function to update Demo 2 sequence with custom behavior
void updateDemo2() {
    ...
    if (currentMillis - stepStartTime >= 50) {  // Update every 50ms or as needed
        Serial.print("Current step: ");
        Serial.println(step);

        switch (step) {
            case 0:
                Serial.println("Centering head, camera, and ears");
                startMovement(117, 90, 90, 90);
                startCameraMovement(90, 90);
                targetRightEarAngle = 90;
                targetLeftEarAngle = 90;
                isEarMoving = true;
                stepStartTime = currentMillis + 2000; // Wait 2 seconds after centering
                step++;
                break;

            case 1:
                Serial.println("Moving to Screenshot 1 angles");
                startMovement(117, 110, 146, 169);  // Head pan 117, tilt 110, Right Ear 146, Left Ear 169
                startCameraMovement(90, 0);         // Camera pan 90, tilt 0
                stepStartTime = currentMillis + 2000; // Wait 2 seconds at these angles
                step++;
                break;

            case 2:
                Serial.println("Moving to Screenshot 2 angles");
                startMovement(78, 106, 77, 180);    // Head pan 78, tilt 106, Right Ear 77, Left Ear 180
                startCameraMovement(27, 55);        // Camera pan 27, tilt 55
                stepStartTime = currentMillis + 1000; // Wait 1 second to settle before wiggle
                step++;
                break;

            case 3:
                Serial.println("Wiggling right ear");
                if (targetRightEarAngle == 77) {
                    targetRightEarAngle = 104;
                } else {
                    targetRightEarAngle = 77;
                }
                isEarMoving = true;

                static int rightEarWiggleCount = 0;
                rightEarWiggleCount++;
                if (rightEarWiggleCount < 6) { // Wiggle 3 times
                    stepStartTime = currentMillis + 500;
                } else {
                    rightEarWiggleCount = 0;
                    step++;
                    stepStartTime = currentMillis + 1000; // Wait 1 second after wiggle
                }
                break;

            case 4:
                Serial.println("Centering everything and waiting");
                startMovement(117, 90, 90, 90);
                startCameraMovement(90, 90);
                stepStartTime = currentMillis + 2000; // Wait 2 seconds after centering
                step++;
            ...
            case 5:
                Serial.println("Moving to Screenshot 3 angles");
                startMovement(171, 109, 148, 90);    // Head pan 171, tilt 109, Right Ear 148, Left Ear 90
                startCameraMovement(159, 0);         // Camera pan 159, tilt 0
                stepStartTime = currentMillis + 1000; // Wait 1 second to settle before wiggle
                step++;
                break;

            case 6:
                Serial.println("Wiggling left ear");
                if (targetLeftEarAngle == 90) {
                    targetLeftEarAngle = 125;
                } else {
                    targetLeftEarAngle = 90;
                }
                isEarMoving = true;

                static int leftEarWiggleCount = 0;
                leftEarWiggleCount++;
                if (leftEarWiggleCount < 6) { // Wiggle 3 times
                    stepStartTime = currentMillis + 500;
                } else {
                    leftEarWiggleCount = 0;
                    step++;
                    stepStartTime = currentMillis + 2000; // Wait 2 seconds after wiggle
                }
                break;

            case 7:
                Serial.println("Centering everything again");
                startMovement(117, 90, 90, 90);
                startCameraMovement(90, 90);
                stepStartTime = currentMillis + 2000; // Wait 2 seconds after centering
                step++;
            ...
            case 8:
                Serial.println("Moving to Screenshot 4 angles");
                startMovement(117, 110, 119, 139);   // Head pan 117, tilt 110, Right Ear 119, Left Ear 139
                startCameraMovement(90, 45);         // Camera pan 90, tilt 45
                stepStartTime = currentMillis + 1000; // Wait 1 second to settle before wiggle
                step++;
                break;

            case 9:
                Serial.println("Wiggling camera tilt");
                if (targetCameraTiltAngle == 25) {
                    targetCameraTiltAngle = 98;
                } else {
                    targetCameraTiltAngle = 25;
                }
                isCameraMoving = true;

                static int cameraTiltWiggleCount = 0;
                cameraTiltWiggleCount++;
                if (cameraTiltWiggleCount < 6) { // Wiggle 3 times
                    stepStartTime = currentMillis + 500;
                } else {
                    cameraTiltWiggleCount = 0;
                    step++;
                    stepStartTime = currentMillis + 2000; // Hang out to conclude demo
                }
                break;

            case 10:
                Serial.println("Ending demo");
                isDemo2Running = false;
                step = 0; // Reset step counter for next run
                break;
        }
