...
void handleMoveCommand(String direction, bool stop) {
  ...
  if (direction == "forward") {
    ...
    moveMotors(-1, 1, leftMotorSpeed, rightMotorSpeed);  // Adjusted for turning
...
void lifeModeBehavior() {
  ...
  if (currentTime - lastStateChange > random(5000, 15000)) {
    currentLifeState = (LifeState)random(0, 3);  
...
void handleObstacles() {
  ...
  switch (obstacleState) {
    ...
    case MOVING_BACKWARD:
      moveMotors(-1, -1, leftMotorSpeed, rightMotorSpeed);
      if (millis() - obstacleStateStartTime > 500) {
...
void smoothStartMotors() {
  ...
  if (currentMillis - lastStepTime >= 20) { 
    lastStepTime = currentMillis;

    if (speed <= targetSpeed) {
      digitalWrite(MOTOR_LEFT_IN1, speed < targetSpeed ? HIGH : LOW);
      digitalWrite(MOTOR_RIGHT_IN1, speed < targetSpeed ? HIGH : LOW);
      speed += increment;
    }
  }
}
...
void checkIfStuck() {
  ...
  if (millis() - lastCheckTime >= checkInterval) {
    if (leftPulseCount == 0 && rightPulseCount == 0) {
      stopMotors();
      Serial.println("Robot is stuck, stopping motors...");
    }
