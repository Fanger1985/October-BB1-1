...
// Function to move servos to specific pose angles
void setPose(int lsa, int lsb, int lwa, int lwb, int lc, int rsa, int rsb, int rwa, int rwb, int rc) {
    leftShoulderA_Angle = lsa;
    leftShoulderB_Angle = lsb;
    leftWristA_Angle = lwa;
    leftWristB_Angle = lwb;
    leftClaw_Angle = lc;

    rightShoulderA_Angle = rsa;
    rightShoulderB_Angle = rsb;
    rightWristA_Angle = rwa;
    rightWristB_Angle = rwb;
    rightClaw_Angle = rc;
...
void introSequence() {
    ...
    setPose(37, 107, 35, 71, 70, 26, 48, 90, 90, 170);
    delay(2000);

    // Second position (screenshot 2)
    setPose(107, 104, 119, 90, 70, 117, 66, 136, 128, 170);
    delay(2000);
    ...
    // Third position (screenshot 3)
    setPose(144, 99, 71, 86, 70, 1, 56, 90, 78, 170);
    delay(2000);

    // Final position (screenshot 4)
    setPose(17, 24, 148, 111, 70, 136, 122, 16, 39, 170);
}
